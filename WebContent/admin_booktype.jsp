...
<c:forEach items="${list}" var="tp" varStatus="index">
				<tr>
					...
					onclick="showInfo2('${tp.tid}','${tp.name}')">修改</button>
					<button type="button" class="btn btn-danger btn-xs"
					onclick="deletebook(${tp.tid})">删除</button></td>
				</tr>
	</c:forEach>
							<%-- <!---在此插入信息-->
							<%
	...
	<script type="text/javascript">
		...
		function deletebook(tid) {
			con = confirm("是否删除?");
			if(con == true) {
				location.href = "/books/deleteBookTypeServlet.tp?tid=" + tid;
			}
		}
	</script>
	<!-------------------------------------------------------------->
	...
	<form class="form-horizontal" method="post"
		action="/books/updateBookTypeServlet.tp">
		<!--保证样式水平不混乱-->
	...
	<!--------------------------------------添加的模糊框------------------------>
	<form class="form-horizontal" method="post"
		action="/books/addBookTypeServlet.tp">
		<!--保证样式水平不混乱-->
		<!-- 模态框（Modal） -->
